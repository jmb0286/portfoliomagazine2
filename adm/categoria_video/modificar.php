...
if(isset($_GET['id'])){
    $idCategoria = $_GET['id'];
    $query = "SELECT * FROM categoria_videos WHERE idCategoriaVideo = '$idCategoria'";
    $resultCategoria = $connection->query($query);
    $categoria = $resultCategoria->fetch_assoc();
    $nombre = $categoria['nombre'];
    $idCategoriaVideo = $categoria['idCategoriaVideo'];
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(empty($nombre) || empty($idCategoria)){
        ...
        $query = "UPDATE categoria_videos SET nombre='$nombre' WHERE idCategoriaVideo = '$idCategoria' ";
