...
$query = "SELECT * FROM categorias WHERE nombreCategoria IN ('grafico','illustrator','indesign','photoshop','Buenas practicas diseño')";
...
$leccionesDgResult = $connection->query($query);

}else{
// LISTADO DE CURSOS FRONT-END
$query = "SELECT * FROM lecciones INNER JOIN detalle_categoria_leccion AS dc ON dc.idLeccion = lecciones.idLeccion WHERE idCategoria = 8";
$leccionesDgResult = $connection->query($query);
}
...
$leccionesDg = randomArray($leccionesDgResult);

$query = "SELECT * FROM codigo_ejemplo WHERE categoria = 'frontend'";
$codigofrontend = $connection->query($query);

$codigoejemplo = [];

$codigoejemplo = randomArray($codigofrontend);


?>


<section class="backend" title="Back-End" style="width:100%;">
        ...
        <form id="formFiltroLecciones">
            <select name="frontend" id="selectCategoria" style="width:100%;">
              ...
              <div class="row pb-3" class="contenedorLecciones">
                <?php
                $contador = 0; 
                  foreach ($leccionesDg as $leccion) {
                    $contador++;
                    if($contador == 7){
                        echo '
...
<script>
  ...
  selectFiltro.addEventListener('change', () => {
    let idCategoria = selectFiltro.value;
        ...
        window.location.href = 'https://portfoliomagazine.info/paginas/portfolio/dgrafico.php?id='+idCategoria;
